...
# Variabel untuk menyimpan nilai konfigurasi
TOKEN = None
USER_IDS = []
IP_CHILLI = None
IP_LAN = None
...
with open(config_file, 'r') as file:
    ...
    for line in lines:
        ...
        if line.startswith('option token'):
            TOKEN = line.split('\'')[1]
        elif line.startswith('option userid'):
            user_ids_str = line.split('\'')[1]
            USER_IDS = [int(user_id.strip()) for user_id in user_ids_str.split(',')]
        elif line.startswith('option username'):
            USERNAME = line.split('\'')[1]
        elif line.startswith('option ip_chilli'):
            IP_CHILLI = line.split('\'')[1]
        elif line.startswith('option ip_lan'):
            IP_LAN = line.split('\'')[1]
        elif line.startswith('option ticket_loc'):
            TICKET_LOC = line.split('\'')[1]
        elif line.startswith('option no_ewallet'):
            NO_EWALLET = line.split('\'')[1]
        elif line.startswith('option geocode'):
            GEOCODE = line.split('\'')[1]

# Validasi konfigurasi
if TOKEN is None:
    print("Token tidak ditemukan dalam file konfigurasi.")
    exit()

if not USER_IDS:
    print("User ID tidak ditemukan dalam file konfigurasi.")
    exit()
...
if IP_CHILLI is None:
    print("IP Chilli tidak ditemukan dalam file konfigurasi.")
    exit()

if TICKET_LOC is None:
    print("Lokasi Print tidak ditemukan dalam file konfigurasi.")
    exit()
    
if IP_LAN is None:
    print("IP LAN tidak ditemukan dalam file konfigurasi.")
    exit()

if NO_EWALLET is None:
    print("No E-Wallet tidak ditemukan dalam file konfigurasi.")
    exit()

if GEOCODE is None:
    print("M4HYCwLHMfS9gL576")
    exit()

with open('/etc/telebot/db.json') as f:
    config = json.load(f)
...
PROFILES_JSON_FILE = "/etc/telebot/profiles.json"
...
quantity_keyboard_data = load_keyboard_quantity('quantity.json')
...
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"""
        keyboard = [
            ...
            [InlineKeyboardButton("HUBUNGI ADMIN", url=f"https://t.me/{USERNAME}")],
        ]
...
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"""
        button = InlineKeyboardButton(text="Dukung Owner", url="https://saweria.co/MutiaraWrt")
        keyboard = InlineKeyboardMarkup([[button]])
        await context.bot.send_message(chat_id=chat_id, text=f"▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬ {commands}", reply_markup=keyboard)
    else:
        welcome_message = f"""▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
        ...
        try:
            ...
            StatusData AS (
                SELECT a.username,
                       ...
                       END AS status
                FROM radacct a
                JOIN LatestAcct la ON a.username = la.username 
                AND a.acctstarttime = la.latest_acctstarttime
            )
            SELECT r.username, COALESCE(sd.status, 'OFFLINE 🔴') AS status
            FROM radcheck r
            LEFT JOIN StatusData sd ON r.username = sd.username;
            """
...
def format_bandwidth(bytes_value):
    ...
    if bytes_value >= 1073741824:  # GB
        return f"{bytes_value / 1073741824:.0f} G"
    elif bytes_value >= 1048576:  # MB
        return f"{bytes_value / 1048576:.0f} M"
...
async def button_callback(update: Update, context: CallbackContext):
    ...
    if data.startswith("view_plan_"):
        ...
        try:
            # Query untuk mengambil data dari billing_plans
            billing_query = """
            SELECT 
                planName, 
                planCost, 
            ...
            conn = get_db_connection()
            with conn.cursor() as cursor:
                cursor.execute(billing_query, (plan_name,))
                billing_details = cursor.fetchone()
                
            if billing_details:
                plan_name = billing_details[0]
                plan_cost = billing_details[1]
                
                # Validasi dan format durasi
                try:
                    plan_timebank_seconds = int(billing_details[2]) if billing_details[2] else 0
                    plan_timebank = format_time(plan_timebank_seconds)
                except ValueError:
                    plan_timebank = 'Invalid'

                # Query untuk mengambil bandwidth dari radgroupreply berdasarkan GroupName
                bandwidth_query = """
                SELECT 
                    value 
                FROM radgroupreply 
                ...
                # Ambil bandwidth max up
                with conn.cursor() as cursor:
                    cursor.execute(bandwidth_query, (plan_name, 'WISPr-Bandwidth-Max-Up'))
                    bandwidth_up = cursor.fetchone()
                
                # Ambil bandwidth max down
                with conn.cursor() as cursor:
                    cursor.execute(bandwidth_query, (plan_name, 'WISPr-Bandwidth-Max-Down'))
                    bandwidth_down = cursor.fetchone()
                
                # Set default values if not found
                plan_bandwidth_up = 'null'
                plan_bandwidth_down = 'null'

                if bandwidth_up:
                    try:
                        plan_bandwidth_up = float(bandwidth_up[0]) if bandwidth_up[0] else 0
                        plan_bandwidth_up = format_bandwidth(plan_bandwidth_up)
                    except ValueError:
                        plan_bandwidth_up = 'Invalid'
                
                if bandwidth_down:
                    try:
                        plan_bandwidth_down = float(bandwidth_down[0]) if bandwidth_down[0] else 0
                        plan_bandwidth_down = format_bandwidth(plan_bandwidth_down)
                    except ValueError:
                        plan_bandwidth_down = 'Invalid'
...
# Fungsi untuk memilih menu voucher (langsung ke durasi)
async def voucher(update: Update, context: CallbackContext):
    user_id = update.effective_user.id

    if is_admin(user_id):
        # Langsung panggil fungsi choose_durasi tanpa opsi CANCEL
...
async def create_voucher(update: Update, context: CallbackContext):
    ...
    link = f"http://{IP_LAN}{TICKET_LOC}?type=batch&plan={groupname}&accounts=Username,Password||{accounts_str}"
...
async def start_topup(update: Update, context: CallbackContext):
    ...
    # Jika sudah dalam proses top-up, reset status
    if context.user_data.get('topup_in_progress', False):
        context.user_data['topup_in_progress'] = False

    # Membuat tombol yang akan memicu pengiriman lokasi
    keyboard = [[InlineKeyboardButton("Alamat Rumah", callback_data='send_location')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
JIKA INGIN TOPUP KURANG DARI
10.000 DATANG LANGSUNG KE
RUMAH OWNER @{USERNAME}
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬""",
        reply_markup=reply_markup
    )
    
    context.user_data['topup_in_progress'] = True
    context.user_data['topup_errors'] = 0 
    
    return TOPUP_AMOUNT

if GEOCODE:
    try:
        latitude_str, longitude_str = GEOCODE.split(',')
        LATITUDE = float(latitude_str.strip())
        LONGITUDE = float(longitude_str.strip())
    except ValueError:
        print("Format geocode salah. Pastikan formatnya adalah 'latitude,longitude'.")
        exit()
...
async def button_click_handler(update: Update, context: CallbackContext):
    ...
    if query.data == 'send_location':
        if LATITUDE is not None and LONGITUDE is not None:
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text="Koordinat lokasi tidak ditemukan.")
...
JIKA SUDAH MELAKUKAN PEMBAYARAN KIRIMKAN BUKTI TRANSFER KE ADMIN : @{USERNAME}
...
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"""
            ...
            bukti = [[InlineKeyboardButton("KIRIM BUKTI", url=f"https://t.me/{USERNAME}")]]
            ...
            # Tandai bahwa topup telah selesai
            context.user_data['topup_in_progress'] = False
            ...
            if error_count >= MAX_ERRORS:
                ...
                context.user_data['topup_in_progress'] = False  # Set ke False karena dibatalkan
                return ConversationHandler.END
            else:
                await update.message.reply_text(
...
async def cancel(update: Update, context: CallbackContext):
    # Mengirim pesan konfirmasi pembatalan
    await update.message.reply_text("Proses topup dibatalkan.")
    
    # Mengubah flag topup_in_progress menjadi False
    context.user_data['topup_in_progress'] = False
